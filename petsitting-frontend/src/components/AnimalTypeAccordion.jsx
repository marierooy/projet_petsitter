...
export function AnimalTypeAccordion({
  ...
  const availableServices = animal.allServices?.filter(
      ...
      {animal.isOpen && (
        <div className="p-4 bg-white border-t">
          ...
          <div className="space-y-3 mb-4">
            <div className="flex items-center space-x-2">
              <Checkbox
                ...
                onChange={(e) => 
                  onUpdateCareMode(index, 'home', e.target.checked)
                }
              />
              <Label htmlFor={`home-${index}`} className="text-sm">
                Garde à domicile
              </Label>
            </div>
            ...
            <div className="flex items-center space-x-2">
              <Checkbox
                ...
                onChange={(e) => 
                  onUpdateCareMode(index, 'sitter', e.target.checked)
                }
              />
              <Label htmlFor={`sitter-${index}`} className="text-sm">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
            <div>
              <Label htmlFor={`animals-${index}`} className="text-sm font-medium">
              ...
              <Input
                ...
                onChange={(e) => onUpdateField(index, 'number_animals', e.target.value)}
                className="mt-1"
              />
            </div>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => onUpdateField(index, 'offer_price', e.target.value)}
                className="mt-1"
              />
            </div>

          {/* Travel Price (only if home care is enabled) */}
          {animal.careModes?.home && (
            <div>
              <Label htmlFor={`travel-${index}`} className="text-sm font-medium">
                Prix déplacement (€)
              </Label>
              <Input
                id={`travel-${index}`}
                ...
                onChange={(e) => onUpdateField(index, 'travel_price', e.target.value)}
          ...
          <div className="border-t pt-4">
            ...
            {/* Current Services */}
            {animal.services?.map((service, serviceIndex) => (
              <div key={service.id} className="p-3 bg-gray-50 rounded-lg mb-3">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => onRemoveService(index, serviceIndex)}
                    className="appearance-none bg-transparent border-none p-0 m-0 text-red-500 hover:bg-transparent hover:text-red-500 focus:outline-none focus:ring-0 active:bg-transparent"
                  >
                    ❌
                  </Button>
                    ...
                    return (
                      <div key={occ.id} className="flex items-center gap-3 h-[50px] overflow-y-auto">
                        <div className="flex items-center space-x-2">
                          ...
                          />
                          <Label 
                            htmlFor={`occ-${animal.id}-${service.id}-${occ.id}`}
                            className="text-sm"
                          >
                            {occ.label}
                          </Label>
            ...
            <div className="flex gap-2 items-end">
              <div className="flex-1">
                ...
                <Select
                  ...
                  onChange={(val) => onSelectService(index, val)}
